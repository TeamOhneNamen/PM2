...
public class StackTest {
	...
	private void createTestData() {
		...
		try {
			Files.lines(Paths.get("data/riesenhasen.txt")).forEach(this.stringStack::push);
			Files.lines(Paths.get("data/riesenhasen.txt")).forEach(this.stringControllStack::addLast);
			Files.lines(Paths.get("data/picasso.txt")).forEach(listToFillAHash.get(rand.nextInt(10))::push);
		} catch (IOException e) {
			e.printStackTrace();
		}
